 ...
 * Fire Behavior Prediction System function
 *
 * @param input The input data, an array of objects containing fuel types, fire weather component, and slope.
 * @param output FBP output options: "Primary", "Secondary", or "All".
 * @param m Optimal number of pixels at each iteration of computation.
 * @param cores Number of CPU cores used in the computation.
 *
 * @return An array of objects with primary, secondary, or all output variables.
 */
...
async function fbp(
    ...
    if (input === null) {
      ...
      input = input.map(record => ({ ...record, FUELTYPE: record.FUELTYPE.toString() }));
      return fireBehaviourPrediction(input);
    } else {
      input = input.map(record => {
      ...
      if (m === null) {
        m = input.length > 500000 ? 3000 : 1000;
      }
      m = input.length >= m ? m : input.length;
      const n0 = Math.round(input.length / m);
      const n = m * n0 >= input.length ? n0 : n0 + 1;
  
      // Set up parallel processing, if # of cores is entered
      if (cores > 1) {
        // Create and register a set of parallel instances for foreach
        const ca = await Promise.all(Array.from({ length: n }, async (_, i) => {
          if (i + 1 === n) {
            return fireBehaviourPrediction(input!.slice((i * m!), input!.length), output);
          } else {
            return fireBehaviourPrediction(input!.slice((i * m!), (i + 1) * m!), output);
          }
        }));
  
        // Flatten the array of arrays
        const fullList = ca.flat();
  
        // Create a single keyed data table
        const keyedList: { [key: string]: any } = fullList.reduce((acc, item) => {
          acc[item.ID] = item;
          return acc;
        }, {});
  
        // Convert to array to keep backwards compatibility
        return Object.values(keyedList);
      } else {
        const ca: Record<string, any>[][] = [];
        for (let i = 0; i < n; i++) {
          let foo;
          if (i + 1 === n) {
            foo = input.slice((i * m!), input.length);
          } else {
            foo = input.slice((i * m!), (i + 1) * m!);
          }
          ca.push(await fireBehaviourPrediction(foo, output));
        }
  ...
  // Dummy function to simulate fire_behaviour_prediction in TypeScript
  async function fireBehaviourPrediction(input: Record<string, any>[], output: string = "Primary"): Promise<Record<string, any>[]> {
    // This function should be implemented based on actual logic of fire_behaviour_prediction
    // For now, it just returns the input array for demonstration purposes
    return input;
  }
